...
<script>
    ...
    export default {
        ...
        methods: {
            getVatsimData() {
                axios.get('/api/flights/vatsim_afv').then(response => {
                    this.vatsimLayer.clearLayers();
                    response.data.pilots.forEach(e => {
                        let marker = L.marker([e.latitude, e.longitude], {icon: vatsimTarget, rotationAngle: e.heading}).addTo(this.vatsimLayer);
                        marker.bindTooltip(e.callsign +'</br>'
                            + e.member.name , {offset: [0, 0], className: 'markerText'});
                    });
                });
